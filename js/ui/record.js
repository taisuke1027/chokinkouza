...
const RecordView = {
  ...
  render() {
    ...
    return el(`
      <div>
        <div class="flex-between" style="margin:8px 0 16px;">
          <h2 style="font-family:var(--font-display); font-size:19px; margin:0;">運動を記録する</h2>
          <input type="date" id="recordDateInput" class="date-input-inline"
        ...
        <div class="card">
          ...
          ${hasQueue ? `<button class="btn-text" id="saveTemplateBtn">💾 よく使うメニューとして保存</button>` : ""}
        </div>
  ...
  renderQueueRow(item) {
    const icon = item.category === "cardio" ? "🏃" : "🏋️";
    return `
