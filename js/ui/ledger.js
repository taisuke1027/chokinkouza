...
const LedgerView = {
  ...
  icons: { cardio: "🫀", strength: "🏋️", endurance: "🔁" },
        ...
        <div class="card">
          ...
          ` : `
            <div class="empty-state">
              <div class="icon">${this.state.toggle === "income" ? "🌱" : "😌"}</div>
        ...
        <div class="card">
          ...
          ${this.state.selectMode ? `
            <div class="flex-between select-action-bar">
              ...
              <button class="btn-secondary" id="bulkDateChangeBtn" style="width:auto; padding:8px 14px; font-size:12.5px;" ${this.state.selectedIds.size === 0 ? "disabled" : ""}>📅 日付を変更</button>
            </div>
          ` : ""}
  ...
  renderDailyRecords(yearMonth) {
    ...
    if (records.length === 0) {
      return `<div class="empty-state"><div class="icon">🗓️</div><p>この月の運動記録はまだありません。</p></div>`;
    }
