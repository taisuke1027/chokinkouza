 ...
 */
const AssetView = {
  state: { mode: "trend", period: "1m", chartType: "stacked" },
  ...
  chartTypes: [
    { key: "stacked", label: "積み上げ面グラフ" },
    { key: "total", label: "総資産のみ" },
    { key: "cardio", label: "心肺のみ" },
    { key: "strength", label: "筋力のみ" },
    { key: "endurance", label: "筋持久力のみ" },
  ],
  ...
  renderTrendBody() {
    ...
    const chartTypeLabel = this.chartTypes.find(c => c.key === this.state.chartType).label;

    return `
      <div class="card">
        <div class="picker-pill-row">
          <button class="picker-pill" id="chartTypePill">📊 ${chartTypeLabel} <span class="caret">▾</span></button>
          <button class="picker-pill" id="periodPill">🗓 ${period.label} <span class="caret">▾</span></button>
        </div>
  ...
  renderChart(filtered) {
    const points = filtered.map(h => ({ date: h.date, values: { cardio: h.cardio, strength: h.strength, endurance: h.endurance, total: h.total } }));
    const layers = [
    ...
    if (this.state.chartType === "stacked") {
      return ChartUI.renderStackedArea(points, layers, { height: 180 });
    }

    const key = this.state.chartType;
    const colorMap = { total: "#A9803F", cardio: this.colors.cardio, strength: this.colors.strength, endurance: this.colors.endurance };
    const seriesPoints = points.map(p => ({ date: p.date, values: { v: p.values[key] } }));
    return ChartUI.renderSVG(seriesPoints, [{ key: "v", color: colorMap[key], showArea: true }], { height: 180 });
  },
  ...
  renderTrendBreakdown(filtered) {
    ...
    return `
      <div class="flex-between" style="padding:7px 0;">
        <span class="small-muted">期間内の変化</span>
        <span class="num" style="font-weight:700; color:${change >= 0 ? "var(--brass-deep)" : "var(--clay)"}">${Fmt.signedBpt(change)} BPT</span>
      </div>
    `;
  },
  ...
  renderList(filtered) {
    if (filtered.length === 0) {
      return `<div class="empty-state"><div class="icon">🗂️</div><p>この期間のデータはまだありません。</p></div>`;
    }
  ...
  afterRender() {
    ...
    if (this.state.mode === "trend") {
      document.getElementById("chartTypePill").addEventListener("click", () => {
        Picker.show("グラフ種類を選択", this.chartTypes, this.state.chartType, (key) => {
          this.state.chartType = key;
          Router.refresh();
        });
      });
