...
const BptCalculator = {
  ...
  processWorkout({ exerciseDef, category, inputFields, seasonId, userId, recordDate }) {
    ...
    asset.cardio += gain.cardio;
    asset.strength += gain.strength;
    asset.endurance += gain.endurance;
    asset.updatedAt = new Date().toISOString();
    Storage.saveAssetForSeason(seasonId, asset);

    Storage.upsertAssetHistoryEntry(
      Models.createAssetHistoryEntry(seasonId, dateKey, asset, gain, {})
    );

    const season = Storage.getSeason(seasonId);
    const newTotal = asset.cardio + asset.strength + asset.endurance;
    const isNewHigh = newTotal > season.highestAsset;
    season.currentAsset = newTotal;
    if (isNewHigh) season.highestAsset = newTotal;
